...
function dice_initialize(container) {
    ...
    set.addEventListener('mousedown', function(ev) { ev.stopPropagation(); });
    set.addEventListener('mouseup', function(ev) { ev.stopPropagation(); });
    set.addEventListener('focus', function(ev) { container.setAttribute('class', ''); });
    set.addEventListener('blur', function(ev) { container.setAttribute('class', 'noselect'); });

    var clearBtn = document.getElementById('clear');
    ['mouseup', 'touchend'].forEach(function(evt) { clearBtn.addEventListener(evt, function(ev) {
        ev.stopPropagation();
        set.value = '0';
        on_set_change();
    }); });
    ...
    if (params.roll) {
        document.getElementById('throw').dispatchEvent(new Event('mouseup', { bubbles: true, cancelable: true }));
    }
    else {
        show_selector();
    }
